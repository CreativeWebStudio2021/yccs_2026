...
$query_ele = "select * from regate_doc WHERE id='$id_comm'";
...
if ($id_canc) {
	if($azione=="sali") $oggetto_frame->sali("$table", "$id_canc", "id_regata", "$id_comm") ;
	if($azione=="scendi") $oggetto_frame->scendi("$table", "$id_canc", "id_regata", "$id_comm") ;
	if($azione=="primo") $oggetto_frame->primo("$table", "$id_canc", "id_regata", "$id_comm") ;
	if($azione=="ultimo") $oggetto_frame->ultimo("$table", "$id_canc", "id_regata", "$id_comm") ;
	if($azione=="cambia") {
		if(isset($_GET['new_pos'])) $new_pos=$_GET['new_pos']; else $new_pos="";
		if($new_pos!="") $oggetto_frame->cambia("$table", "$id_canc", "$new_pos", "id_regata", "$id_comm") ;	
	}
	/*
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){
		...
		<script type="text/javascript">
			window.location='http://<?php echo $ind_sito?>/resarea/frame/link_comunicati.php?id_comm=<?php echo $id_comm;?>';
		</script>
	<?php 	
}
...
if($azione=="visibile") {
	...
	?>
	<script type="text/javascript">
		window.location='<?php echo $http;?>://<?php echo $ind_sito?>/resarea/frame/regate_doc.php?id_comm=<?php echo $id_comm;?>';
	</script>
	<?php 
}
...
<script type="text/javascript">
	...
	function aggiungi_lista(id_check, id_campo){
		...
		if(lista_del!=""){ 
			...
			document.getElementById('cancella_sel').href='<?php echo $http;?>://<?php echo $ind_sito?>/resarea/frame/<?php echo $file;?>.php?azione=cancella_sel&id_comm=<?php echo $id_comm;?>&lista='+lista_del;
		}else{
...
<body style="background:#fff">
	...
	<div class="mws-panel grid_8">
		<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Documenti Regate - Link</b></div>
		...
		<div id="inserisci">
			<div class="mws-panel-header">
				<span>Inserisci Documenti Regate</span>
			</div>
			<div class="mws-panel-body no-padding" style="margin-top:40px">
				<form class="mws-form"  action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" name="invia_comm" enctype="multipart/form-data" style="margin-top:-40px">			
					...
					<div class="mws-form-inline">
						...
						<?php 
						$ord = $oggetto_admin->trova_ordine("$table", "id_regata", "$id_comm");
		...
		<?php 
		$criterio = " AND id_regata='$id_comm'";
		?>
