...
$query="SELECT id_edizione FROM edizioni_info WHERE id='$id_campo'";
$resu=$open_connection->connection->query($query);
list($id_edizione)=$resu->fetch();

$query_up="UPDATE edizioni_info SET programma='0' WHERE id_edizione='$id_edizione'";
//echo $query_up;
$risu_up=$open_connection->connection->query($query_up);

if($val==1){
	$query_up="UPDATE edizioni_info SET programma='$val' WHERE id='$id_campo'";
	//echo $query_up;
	$risu_up=$open_connection->connection->query($query_up);
}
?>