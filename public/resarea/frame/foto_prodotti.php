...
$rif="";
if($id_prod!="") $rif="&id_prod=$id_prod";

$query_ele = "select * from prodotti WHERE id='$id_prod'";
...
if ($id_canc) {
	if($azione=="sali") $oggetto_frame->sali("$table", "$id_canc", "id_rife", "$id_prod") ;
	if($azione=="scendi") $oggetto_frame->scendi("$table", "$id_canc", "id_rife", "$id_prod") ;
	if($azione=="primo") $oggetto_frame->primo("$table", "$id_canc", "id_rife", "$id_prod") ;
	if($azione=="ultimo") $oggetto_frame->ultimo("$table", "$id_canc", "id_rife", "$id_prod") ;
	if($azione=="cambia") {
		if(isset($_GET['new_pos'])) $new_pos=$_GET['new_pos']; else $new_pos="";
		if($new_pos!="") $oggetto_frame->cambia("$table", "$id_canc", "$new_pos", "id_rife", "$id_prod") ;	
	}
	/*
...
{	
	...
	if ($risu_canc_img) {
		...
		if (is_file("../img_up/prodotti/$foto")) @unlink("../img_up/prodotti/s_$foto");
	}
...
<body style="background:#fff">
	...
	<div class="mws-panel grid_8">
		<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Foto Prodotto - <?php echo $risu_ele['nome'];?></b></div>
			...
			<div class="mws-panel-body no-padding" style="margin-top:40px">
				<form class="mws-form"  action="<?php echo $_SERVER['REQUEST_URI'];?>" method="post" name="invia_foto" enctype="multipart/form-data" style="margin-top:-40px">			
					...
					<div class="mws-form-inline">
						...
						<?php 
						$ord = $oggetto_admin->trova_ordine("$table", "id_rife", "$id_prod");
			...
			<table class="mws-datatable-fn mws-table">
				<thead>
					<tr>
						...
						<th></th>
						<th style="text-align:left; width:200px;">Azioni</th>
					</tr>
				</thead>
				<tbody>
				<?php 
