...
if ($id_immagine) {
	...
	if($azione=="cancella"){
		...
		if ($risu_canc_img) {
			...
			if (is_file("img_up/magazine/$logo")) @unlink("img_up/magazine/$logo");
			if (is_file("img_up/magazine/s_$logo")) @unlink("img_up/magazine/s_$logo");
		}
...
<div style="padding:10px 20px 20px 20px">
	...
	<div class="mws-panel-body no-padding">
		<table class="mws-datatable-fn mws-table">
			...
			<tbody>
			<?php 
				$query_ele = "SELECT * FROM magazine_gallery WHERE id_gallery = '$id_gallery' ORDER BY ordine DESC";
				//echo $query_ele;
				...
				{						  
					...
					$query_ele = "SELECT * FROM magazine_gallery WHERE id_gallery = '$id_gallery' ORDER BY ordine DESC LIMIT $start,$rec_pag";
					$risu_ele = $open_connection->connection->query($query_ele);
					...
					{						
						...
						//$link=$http.'://'.$ind_sito.'/resarea/img_up/magazine/'.$immagine;
						$link="/resarea/img_up/magazine/'.$immagine";
			?>
				<tr>
					<td align="center" valign="center"><?php  echo $start+$x+1; ?></td>
					<td style="position:relative; width:150px">
						<img src="img_up/magazine/<?php if(is_file("img_up/magazine/s_".$immagine)) echo "s_";?><?php  echo $immagine; ?>" style="width:150px"/>
					</td>
					<td>
						<?php  echo $link; ?>
