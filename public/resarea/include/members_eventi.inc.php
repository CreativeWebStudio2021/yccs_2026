...
{	
	...
	$query_canc_img = "select allegato, allegato_eng from members_eventi_link where id_rife='$id_canc'";
	...
	$query_canc = "delete from members_eventi_link where id_rife='$id_canc'";
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){
		$query_canc_img = "select allegato, allegato_eng from members_eventi_link where id_rife='".$temp[$z]."'";
		$risu_canc_img = $open_connection->connection->query($query_canc_img);
		if ($risu_canc_img) {
			list($allegato, $allegato_eng) = $risu_canc_img->fetch();
			if (is_file("files/$allegato")) @unlink("files/$allegato");
		...
		$query_canc = "delete from members_eventi_link where id_rife='".$temp[$z]."'";
		$risu_canc = $open_connection->connection->query($query_canc);
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Eventi Young Azzurra</b></div>
	...
	<div class="mws-panel-header">
		<span><i class="icon-table"></i> Elenco Eventi</span>
	</div>
		...
		<table class="mws-datatable-fn mws-table">
			...
			?>	
				...
				<tr>
					...
					<td align="left">
						<?php if($arr_ele['testo_gruppo'] && $arr_ele['testo_gruppo']!=""){?><img src="../images/it.gif" alt="" style="width:10px;"/>&nbsp;<?php  echo $tit_gruppo; ?><br/><?php }?>
						<?php if($arr_ele['testo_gruppo_eng'] && $arr_ele['testo_gruppo_eng']!=""){?><img src="../images/en.gif" alt="" style="width:10px;"/>&nbsp;<?php  echo $tit_gruppo_eng; ?><?php }?>	
						<br/>
						<?php if($arr_ele['luogo'] && trim($arr_ele['luogo']!="")) echo $arr_ele['luogo'];?>
						<?php if($arr_ele['data'] && trim($arr_ele['data']!="" && $arr_ele['data']!="0000-00-00")) echo " ".date_to_data($arr_ele['data']);?>
						<?php if($arr_ele['data_al'] && trim($arr_ele['data_al']!="" && $arr_ele['data_al']!="0000-00-00")) echo " / ".date_to_data($arr_ele['data_al']);?>
					</td>
					<td align="center">
						<?php if($arr_ele['tipo']=="gruppo_link"){
							$query_comm="SELECT id FROM members_eventi_link WHERE id_rife='$id_campo'";
							$resu_comm=$open_connection->connection->query($query_comm);
							$num_comm=$resu_comm->rowCount();
							?>
							<span style="color:#000; cursor:pointer;" onclick="vedi_link('<?php echo $id_campo;?>')" class="btn btn-sm"><?php echo $num_comm;?></span>
						<?php }?>
					</td>
					<td style="width:10%">
...
<script type="text/javascript">
	function vedi_link(id_comm){
		$("#box_link").fadeIn();
		document.getElementById('frame_link').src="frame/link_members_eventi.php?id_comm="+id_comm;	
	}
</script>
