...
if($azione=="cancella" && $id_canc!="") {
	$query_canc_doc = "select pdf,pdf_eng from members_calendario where id='$id_canc'";
	...
	$query_canc = "delete from members_calendario where id='$id_canc'";
	$risu_canc = $open_connection->connection->query($query_canc);
	
?>
	<script language="javascript">		
		window.alert("Il campo e' stato cancellato con successo");
		window.location="admin.php?cmd=members_calendario<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>";
	</script>
<?php 
} 
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){		
		$query_canc_doc = "select pdf,pdf_eng from members_calendario where id='".$temp[$z]."'";
		$risu_canc_doc = $open_connection->connection->query($query_canc_doc);
		if ($risu_canc_doc) {
			$num_canc_doc = $risu_canc_doc->rowCount();
			for ($f=0; $f<$num_canc_doc; $f++) {
		...
		$query_canc = "delete from members_calendario where id='".$temp[$z]."'";
		$risu_canc = $open_connection->connection->query($query_canc);
	}?>
...
<script type="text/javascript">
	...
	function aggiungi_lista(id_check, id_campo){
		...
		if(lista_del!=""){
			...
			document.getElementById('cancella_sel').href='admin.php?cmd=members_calendario<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_del;
		}else{
	...
	function aggiugni_tutti(){
		...
		if(document.getElementById('check_tutti').checked){
			...
			document.getElementById('cancella_sel').style.display="block";
			document.getElementById('cancella_sel').href='admin.php?cmd=members_calendario<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_tutti;
		}else{
			lista_tutti="";
			ind_check = 1;
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Calendario Young Azzurra</b></div>
	
	<div id="start" style="display:none"></div>
	<div id="end" style="display:none"></div>
	<div id="total" style="display:none"></div>
	
	<?php $query_ele = "select * from members_calendario WHERE 1 $criterio";
	...
	?>
		...
		<?php if($num_ele==0){?>
			...
			<div style="float:left;width:50%;height:30px;text-align:right"><a href="admin.php?cmd=members_calendario_ins<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" style="color:#7a7a7a"><b>Aggiungi documento</b></a> &nbsp;</div>
		<?php }?>
		...
		<table class="mws-datatable-fn mws-table">
			...
			<?php 
				...
				{					  
					...
					$query_ele = "select * from members_calendario WHERE 1 $criterio LIMIT $start,$rec_pag";
					$risu_ele = $open_connection->connection->query($query_ele);
				...
				<tr>
					...
					<td style="width:10%" align="center" valign="center">
						<span class="btn-group">
							...
							<a href="admin.php?cmd=members_calendario_mod&id_rec=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-pencil"></i></a>
							<a href="admin.php?cmd=members_calendario&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
