...
<script language="javascript">
	function annulla(){
		window.location='admin.php?cmd=documenti<?php echo $rif;?>';
	}
</script>
...
if($campocanc!="")
{
	...
	if(is_file("files/regate/doc/$cancimg")){unlink("files/regate/doc/$cancimg");}
	
	$query_canc_img = "update $table set $campocanc='' where id='$id_rec'";
	$open_connection->connection->query($query_canc_img);
?>
	<script language="javascript">
		window.location='admin.php?cmd=documenti_mod&id_rec=<?php  echo $id_rec; ?><?php echo $rif;?>';
	</script>	
<?php 
}
...
if($stato=="inviato")
{
	...
	$oggetto_admin->modifica_campi ("$table" ,$id_rec , $arr_no ,  $arr_thumb="no", "", "files/regate/doc");		
?>
	<script language="javascript">
		window.location='admin.php?cmd=documenti<?php echo $rif;?>';
	</script>	
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px">Modifica Documento della regata <b><?php  echo ucfirst($nome_reg); ?></b> - <b>Edizione <?php  echo $anno_ed; ?></b></div>
	
	<div style="display:flex; justify-content:space-between;">
		<a href="admin.php?cmd=documenti<?php echo $rif;?>" style="color:#7a7a7a">
	...
	<div class="mws-panel-body no-padding">
		<form name="inserimento" class="mws-form" action="admin.php?cmd=documenti_mod&id_rec=<?php  echo $id_rec; ?><?php echo $rif;?>" method="post" enctype="multipart/form-data">
			...
			<div class="mws-form-inline">
				...
				<div class="mws-form-row">
					...
					<div class="mws-form-item">
						...
						<a href="admin.php?cmd=documenti_mod&id_rec=<?php  echo $id_rec; ?>&campocanc=testo_link<?php echo $rif;?>"><i style="font-size:1.5em;" class="fa fa-eraser" aria-hidden="true"></i></a>
					</div>
				</div>
				...
				<div id="box_link" style="display:<?php if($n_tipo_link=="link"){?>block<?php }else{?>none<?php }?>">
					<div class="mws-form-row">
						...
						<div class="mws-form-item">
							...
							<?php if($n_link_eng && $n_link_eng!=""){?><a href="admin.php?cmd=documenti_mod&id_rec=<?php  echo $id_rec; ?>&campocanc=link_eng<?php echo $rif;?>"><i style="font-size:1.5em;" class="fa fa-eraser" aria-hidden="true"></i></a><?php }?>
						</div>
					</div>
					<div class="mws-form-row">
						...
						<div class="mws-form-item">
							...
							<?php if($n_link && $n_link!=""){?><a href="admin.php?cmd=documenti_mod&id_rec=<?php  echo $id_rec; ?>&campocanc=link<?php echo $rif;?>"><i style="font-size:1.5em;" class="fa fa-eraser" aria-hidden="true"></i></a><?php }?>
						</div>
					</div>
				</div>
				<div id="box_allegato" style="display:<?php if($n_tipo_link=="allegato"){?>block<?php }else{?>none<?php }?>">
					<?php 
					$oggetto_admin->campo_mod("Allegato PDF (Inglese)" , "file_eng" , "$n_file_eng"  , "5", 'no', "$cmd", "$id_rec$rif", "", "", "", "files/regate/doc");
					$oggetto_admin->campo_mod("Allegato PDF (Italiano)" , "file" , "$n_file"  , "5", 'no', "$cmd", "$id_rec$rif", "", "", "", "files/regate/doc");
					?>
				</div>
