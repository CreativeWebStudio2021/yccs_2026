...
{	
	...
	$query_canc_iscr = "select file,file_eng from edizioni_iscritti where id='$id_canc'";
	...
	$query_canc = "delete from edizioni_iscritti where id='$id_canc'";
	$risu_canc = $open_connection->connection->query($query_canc);
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){		
		$query_canc_iscr = "select file,file_eng from edizioni_iscritti where id='".$temp[$z]."'";
		$risu_canc_iscr = $open_connection->connection->query($query_canc_iscr);
		if ($risu_canc_iscr) {
			$num_canc_iscr = $risu_canc_iscr->rowCount();
			for ($f=0; $f<$num_canc_iscr; $f++) {
		...
		$query_canc = "delete from edizioni_iscritti where id='".$temp[$z]."'";
		$risu_canc = $open_connection->connection->query($query_canc);
	}?>
		<script type="text/javascript">
			window.location='admin.php?cmd=iscritti<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>';
...
<script type="text/javascript">
	...
	function aggiungi_lista(id_check, id_campo){
		...
		if(lista_del!=""){
			...
			document.getElementById('cancella_sel').href='admin.php?cmd=iscritti<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_del;
		}else{
	...
	function aggiugni_tutti(){
		...
		if(document.getElementById('check_tutti').checked){
			...
			document.getElementById('cancella_sel').style.display="block";
			document.getElementById('cancella_sel').href='admin.php?cmd=iscritti<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_tutti;
		}else{
			lista_tutti="";
			ind_check = 1;
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px">Iscritti della regata <b><?php  echo ucfirst($nome_reg); ?></b> - <b>Edizione <?php  echo $anno_ed; ?></b></div>
	...
	<div style="display:flex; justify-content:space-between;">
		...
		<a href="admin.php?cmd=iscritti_ins<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>">
	...
	<div class="mws-panel-header" style="clear:both">
		<span><i class="icon-table"></i> Elenco iscritti</span>
	</div>
		...
		<table class="mws-datatable-fn mws-table">
			<thead>
				<tr>
					...
					<?php /*<th style="text-align:left;">Link Univoco</th>*/?>
					<th>Tipo</th>
					<th>Link Fisso</th>
					<th>Azioni</th>
				</tr>
			...
			<tbody>
			<?php 
				$query_ele = "select * from edizioni_iscritti where id_regata='$id_rife' and id_edizione='$id_riferimento' order by ordine desc";
				...
				{					  
					...
					$query_ele = "select * from edizioni_iscritti where id_regata='$id_rife' and id_edizione='$id_riferimento' order by ordine desc LIMIT $start,$rec_pag";
					$risu_ele = $open_connection->connection->query($query_ele);
				...
				<tr>
					...
					<td align="center" style="width:500px;">
						<?php if($tipo=="allegato"){?>
							...
							<div style="width:500px; display:<?php if($fisso==1){?>block<?php }else{?>none<?php }?>" id="links_<?php echo $x;?>">
								...
								$link_ita = $http."://".$ind_sito."/regate-".$anno_ed."/".to_htaccess_url($nome_reg,"")."-".$id_riferimento."/iscritti-".$id_campo."/".to_htaccess_url($tit,"");
								$link_eng = $http."://".$ind_sito."/en/regattas-".$anno_ed."/".to_htaccess_url($nome_reg,"")."-".$id_riferimento."/iscritti-".$id_campo."/".to_htaccess_url($tit_eng,"");
								?>
							...
							<script>
								...
								function linkFisso_<?php echo $x;?>(){
									if(document.getElementById("fisso_<?php echo $x;?>").innerHTML=='<i class="fa fa-circle" style="color:green"></i>'){									
										...
										document.getElementById('hiddenFrame').src="frame/link_fisso.php?tabella=edizioni_iscritti&id_campo=<?php echo $id_campo;?>&val=0";
									}else{
										...
										document.getElementById('hiddenFrame').src="frame/link_fisso.php?tabella=edizioni_iscritti&id_campo=<?php echo $id_campo;?>&val=1";
									}
								}
							</script>
						<?php }?>
					...
					<?php /*<td align="center">
						<span class="btn-group">
							<span class="btn btn-small btn-fix" id="btn-fix-<?php echo $id_campo;?>" onclick="fissaLink('<?php echo $id_campo;?>','<?php echo $tipo; ?>');" style="cursor:pointer; <?php if($arr_ele['link_fisso']==1){?>color:rgb(0, 128, 0)<?php }?>" title="Link <?php if($arr_ele['link_fisso']==0){?>non <?php }?>fissato"><i class="fa fa-circle"></i></span>
						</span>
					</td>*/?>
					<td style="width:10%" align="center" valign="center">
						<span class="btn-group">
							<a href="admin.php?cmd=iscritti&id_canc=<?php  echo $id_campo; ?>&azione=primo<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up-2"></i></a>
							...
							<a href="admin.php?cmd=iscritti_mod&id_rec=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-pencil"></i></a>
							<a href="admin.php?cmd=iscritti&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
