...
//if($cat_ric!=""){$criterio .= " AND id_sottocat='$cat_ric'"; $rif.="&cat_ric=$cat_ric";}
if($macro_ric!=""){$criterio .= " AND id_cat='$macro_ric'"; $rif.="&macro_ric=$macro_ric";}
if($cat_ric!=""){$criterio .= " AND id_sottocat='$cat_ric'"; $rif.="&cat_ric=$cat_ric";}
	
if($azione=="cancella" && isset($id_canc) && $id_canc!="")
{		
	/*$query_canc_img = "select img from $table where id='$id_canc'";
	$risu_canc_img = $open_connection->connection->query($query_canc_img);
	if (isset($risu_canc_img)) {
...
if($azione=="cambia_stato") {
	...
	$query_up="UPDATE magazine_stato SET stato='$new_stato' WHERE id='1'";
	echo $query_up;
...
<div class="mws-panel grid_8">
	...
	<div style="width:100%; height:40px; margin-bottom:5px;">
		...
		<div style="float:right; margin-top:5px; margin-right:10px;">
			<?php if($stato=="1"){?><span style="color:green"><b>MAGAZINE VISIBILE</b></span><?php }else{?><span style="color:red"><b>MAGAZINE NON VISIBILE</b></span><?php }?>
		</div>
	</div>
	...
	<div style="display:flex; justify-content:space-between; align-items:center;">
		<div style="height:30px;display:flex; gap:10px;">
			...
			<div style=" margin-top:-5px; ">
				<select name="macro_ric" class="small" onchange="window.location='admin.php?cmd=magazine_articolo&macro_ric='+this.value">
			...
			<div style="margin-top:-5px;">
				<select name="cat_ric" class="small" onchange="window.location='admin.php?cmd=magazine_articolo&macro_ric=<?php echo $macro_ric;?>&cat_ric='+this.value">
					<option value="">Seleziona</option>
					<?php 
					$query_c="SELECT * FROM magazine_categorie WHERE id_cat='$macro_ric' ORDER BY ordine DESC";
					$resu_c=$open_connection->connection->query($query_c);
					while($risu_c=$resu_c->fetch()){?>
						<option value="<?php echo $risu_c['id'];?>" <?php if($risu_c['id']==$cat_ric){?>selected="selected"<?php }?>><?php echo $risu_c['nome'];?></option>
					<?php }?>					
				</select>
			</div>								
			...
			<?php 
				...
				{		
					...
					{						
						...
						if($id_cat){
							$query_c="SELECT nome FROM magazine_macrocategorie WHERE id='$id_cat'";
							$resu_c=$open_connection->connection->query($query_c);
							list($nome_cat)=$resu_c->fetch();
						}
						
						if($id_sottocat){
							$query_c2="SELECT nome FROM magazine_categorie WHERE id='$id_sottocat'";
							$resu_c2=$open_connection->connection->query($query_c2);
							list($nome_sottocat)=$resu_c2->fetch();
						}
						
						if(file_exists("img_up/magazine/s_$foto")) $ante = "img_up/magazine/s_$foto";
						elseif(file_exists("img_up/magazine/$foto")) $ante = "img_up/magazine/$foto";
						else $ante = "https://www.yccs.it/resarea/img_up/magazine/$foto";
			?>	
				<script type="text/javascript">
				...
				<tr>
					...
					<td>
						...
						if($id_cat) $link_magazine.=to_htaccess_url($nome_cat,"")."/";							
						if($id_sottocat) $link_magazine.=to_htaccess_url($nome_sottocat,"")."/";							
						$link_magazine.=to_htaccess_url($tit,"");
						$link_magazine.="-".$id_campo.".html";
						
						$link_magazine_code = "$http://$ind_sito/magazine/$codice-$id_campo";
						?>
						
						<div id="testo_link_<?php echo $id_campo;?>" style="<?php if($stato==1 && $visibile=='1'){?>display:none;<?php }?>">
							<input type="text" id="link_<?php echo $id_campo;?>" style="width:80%" readonly value="<?php echo $link_magazine_code;?>"/><a class="btn btn-small"  style="cursor:pointer;" onclick="copyLink('<?php echo $id_campo;?>');"><i class="fa fa fa-clipboard" aria-hidden="true"></i></a>
						</div>
					...
					<td style="width:350px">
						...
						<span class="btn-group">
							...
							<a class="btn btn-small" href="<?php echo $link_magazine;?>" target="_blank"><i class="fa fa-external-link" aria-hidden="true"></i></a>
						</span>
						<span class="btn-group">
							...
							<a onclick="return confirm('Cancellare l\'articolo?')" href="admin.php?cmd=<?php echo $table;?>&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php echo $rif;?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
...
<script>
  ...
  function visibile(id){
    ...
    document.getElementById('hiddenFrame').src="frame/magazine_cambiaVisibilita.php?id_articolo="+id;
  }
  ...
  function evidenza(id){
    ...
    document.getElementById('hiddenFrame').src="frame/magazine_cambiaEvidenza.php?id_articolo="+id;
  }
