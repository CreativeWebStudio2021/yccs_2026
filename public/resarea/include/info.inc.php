...
{	
	...
	$query_canc_info = "select file,file_eng from edizioni_info where id='$id_canc'";
	...
	$query_canc = "delete from edizioni_info where id='$id_canc'";
	$risu_canc = $open_connection->connection->query($query_canc);
	
?>
	<script language="javascript">		
		window.location="admin.php?cmd=info<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>";
	</script>
<?php 
} 

if ($id_canc) {
	if($azione=="sali") $oggetto_admin->sali("edizioni_info", "$id_canc", "id_regata" , "$id_rife", "id_edizione", "$id_riferimento") ;
	if($azione=="scendi") $oggetto_admin->scendi("edizioni_info", "$id_canc", "id_regata" , "$id_rife", "id_edizione", "$id_riferimento") ;
	if($azione=="primo") $oggetto_admin->primo("edizioni_info", "$id_canc", "id_regata" , "$id_rife", "id_edizione", "$id_riferimento");
	if($azione=="ultimo") $oggetto_admin->ultimo("edizioni_info", "$id_canc", "id_regata" , "$id_rife", "id_edizione", "$id_riferimento");	
	if($azione=="cambia") {
		if(isset($_GET['new_pos'])) $new_pos=$_GET['new_pos']; else $new_pos="";
		if($new_pos!="") $oggetto_admin->cambia("edizioni_info", "$id_canc", "$new_pos", "id_regata" , "$id_rife", "id_edizione", "$id_riferimento");			
	}
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){		
		$query_canc_info = "select file,file_eng from edizioni_info where id='".$temp[$z]."'";
		$risu_canc_info = $open_connection->connection->query($query_canc_info);
		if ($risu_canc_info) {
			$num_canc_info = $risu_canc_info->rowCount();
			for ($f=0; $f<$num_canc_info; $f++) {
		...
		$query_canc = "delete from edizioni_info where id='".$temp[$z]."'";
		$risu_canc = $open_connection->connection->query($query_canc);
	}?>
		<script type="text/javascript">
			window.location='admin.php?cmd=info<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>';
...
<script type="text/javascript">
	...
	function aggiungi_lista(id_check, id_campo){
		...
		if(lista_del!=""){
			...
			document.getElementById('cancella_sel').href='admin.php?cmd=info<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_del;
		}else{
	...
	function aggiugni_tutti(){
		...
		if(document.getElementById('check_tutti').checked){
			...
			document.getElementById('cancella_sel').style.display="block";
			document.getElementById('cancella_sel').href='admin.php?cmd=info<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_tutti;
		}else{
			lista_tutti="";
			ind_check = 1;
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px">Info della regata <b><?php  echo ucfirst($nome_reg); ?></b> - <b>Edizione <?php  echo $anno_ed; ?></b></div>
	...
	<div style="display:flex; justify-content:space-between;">
		...
		<a href="admin.php?cmd=info_ins<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>">
	...
	<div class="mws-panel-header" style="clear:both">
		<span><i class="icon-table"></i> Elenco info</span>
	</div>
	<div class="mws-panel-body no-padding">
		<table class="mws-datatable-fn mws-table">
			...
			<tbody>
			<?php 
				$query_ele = "select * from edizioni_info where id_regata='$id_rife' and id_edizione='$id_riferimento' order by ordine desc";
				...
				{					  
					...
					$query_ele = "select * from edizioni_info where id_regata='$id_rife' and id_edizione='$id_riferimento' order by ordine desc LIMIT $start,$rec_pag";
					$risu_ele = $open_connection->connection->query($query_ele);
			...
			?>
				...
				<tr>
					...
					<td align="center" style="width:500px;">
						<?php if($tipo=="allegato"){?>
							...
							<div style="width:500px; display:<?php if($fisso==1){?>block<?php }else{?>none<?php }?>" id="links_<?php echo $x;?>">
								...
								$link_ita = $http."://".$ind_sito."/regate-".$anno_ed."/".to_htaccess_url($nome_reg,"")."-".$id_riferimento."/info-".$id_campo."/".to_htaccess_url($tit,"");
								$link_eng = $http."://".$ind_sito."/en/regattas-".$anno_ed."/".to_htaccess_url($nome_reg,"")."-".$id_riferimento."/info-".$id_campo."/".to_htaccess_url($tit_eng,"");
								?>
							...
							<script>
								...
								function linkFisso_<?php echo $x;?>(){
									if(document.getElementById("fisso_<?php echo $x;?>").innerHTML=='<i class="fa fa-circle" style="color:green"></i>'){									
										...
										document.getElementById('hiddenFrame').src="frame/link_fisso.php?tabella=edizioni_info&id_campo=<?php echo $id_campo;?>&val=0";
									}else{
										...
										document.getElementById('hiddenFrame').src="frame/link_fisso.php?tabella=edizioni_info&id_campo=<?php echo $id_campo;?>&val=1";
									}
								}
							</script>
						<?php }?>
					...
					<td align="center">
						<span id="prog_<?php echo $x;?>" style="cursor:pointer;" onclick="visibFunct_<?php echo $x;?>();"><i class="fa fa-circle" style="color:<?php if($programma==1){?>green<?php }else{?>grey<?php }?>"></i></span>
						<script>
							var prog_<?php echo $x;?>="<?php echo $programma;?>";
							function visibFunct_<?php echo $x;?>(){
								for(var i=0; i<=50; i++){	
									if(document.getElementById('prog_'+i) && i!=<?php echo $x;?>) {
										$("#prog_"+i).html('<i class="fa fa-circle" style="color:grey"></i>');
										prog = '0';
									}
								}
								if(document.getElementById("prog_<?php echo $x;?>").innerHTML=='<i class="fa fa-circle" style="color:green"></i>')
								{
								//if(prog_<?php echo $x;?>=="1"){
									//prog_<?php echo $x;?>=0;
									$("#prog_<?php echo $x;?>").html('<i class="fa fa-circle" style="color:grey"></i>');
									document.getElementById('hiddenFrame').src="frame/programma.php?id_campo=<?php echo $id_campo;?>&val=0";
								}else{
									//prog_<?php echo $x;?>=1;
									$("#prog_<?php echo $x;?>").html('<i class="fa fa-circle" style="color:green"></i>');
									document.getElementById('hiddenFrame').src="frame/programma.php?id_campo=<?php echo $id_campo;?>&val=1";
								}
							}
						</script>
					</td>
					<td style="width:10%" align="center" valign="center">
						<span class="btn-group">
							<a href="admin.php?cmd=info&id_canc=<?php  echo $id_campo; ?>&azione=primo<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up-2"></i></a>
							<a href="admin.php?cmd=info&id_canc=<?php  echo $id_campo; ?>&azione=sali<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up"></i></a>
							<a href="admin.php?cmd=info&id_canc=<?php  echo $id_campo; ?>&azione=scendi<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-down"></i></a>
							<a href="admin.php?cmd=info&id_canc=<?php  echo $id_campo; ?>&azione=ultimo<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-down-2"></i></a>
							...
							<a href="admin.php?cmd=info_mod&id_rec=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-pencil"></i></a>
							<a href="admin.php?cmd=info&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
