...
{	
	...
	$query_canc_img = "select img,video from $table where id='$id_canc'";
	$risu_canc_img = $open_connection->connection->query($query_canc_img);
	if ($risu_canc_img) {
		list($img,$video) = $risu_canc_img->fetch();
		if (is_file("img_up/ya_slide/$img")) @unlink("img_up/ya_slide/$img");
		if (is_file("img_up/ya_slide/s_$img")) @unlink("img_up/ya_slide/s_$img");
		if (is_file("files/ya_slide/$video")) @unlink("files/ya_slide/$video");
	}
...
if ($id_canc) {
	...
	if($azione=="attiva") $open_connection->connection->query("update ya_slide set visibile='1' where id='$id_canc'") ;
	if($azione=="disattiva") $open_connection->connection->query("update ya_slide set visibile='0' where id='$id_canc'") ;
}
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Slide Young Azzurra</b></div>
	...
	<div class="mws-panel-header">
		<span><i class="icon-table"></i> Elenco Slide Young Azzurra</span>
	</div>
		...
		<table class="mws-datatable-fn mws-table">
			<thead>
				<tr>
					...
					<th style="width:150px">Foto/Video</th>
					<th style="text-align:left;">Didascalia</th>
					<!--<th>Link</th>-->
					<th>Visibile</th>
					<th>Azioni</th>
				</tr>
			...
			<?php 
				...
				{					  
					...
					{						
						...
						$dida = $riga1."<br>".$riga2."<br>".$riga3;
						
						if(file_exists("img_up/ya_slide/s_$foto")) $ante = "img_up/ya_slide/s_$foto";
						elseif(file_exists("img_up/ya_slide/$foto")) $ante = "img_up/ya_slide/$foto";
						else $ante = "https://www.yccs.it/resarea/img_up/ya_slide/$foto";
			?>
				<script type="text/javascript">
				...
				<tr>
					...
					<td style="position:relative; width:150px;">
						...
						<?php  if ($video!="" && is_file("files/ya_slide/$video")) { ?><div style="position:absolute;top:40%;left:43%"><i class="icon-play" style="color:#000;font-size:2.0em"></i></div><?php  } ?>
					</td>
					<td><?php  echo $dida; ?></td>
					<!--<td><?php  echo $link; ?></td>-->
					<td align="center" valign="center">
					<?php   
						if ($vis==1) {?> 
							<a href="admin.php?cmd=ya_slide&azione=disattiva&id_canc=<?php echo $id_campo.$rif;?>&pag_att=<?php echo $pag_att;?>" title="Rendi non visibile">
								<i class="fa fa-check-circle fa-2x" aria-hidden="true"></i>
							</a>
					<?php }else{?>
							<a href="admin.php?cmd=ya_slide&azione=attiva&id_canc=<?php echo $id_campo.$rif;?>&pag_att=<?php echo $pag_att;?>" title="Rendi visibile">
					...
					<td style="width:10%">
						<span class="btn-group">
							...
							<a onclick="return confirm('Desideri cancellare l\'elemento?');" href="admin.php?cmd=<?php echo $table;?>&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php echo $rif;?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
