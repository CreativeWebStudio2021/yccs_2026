...
{		
	...
	if ($risu_canc_img) {
		...
		if (is_file("img_up/azzurra_pagine_slide/$foto")) @unlink("img_up/azzurra_pagine_slide/$foto");
		if (is_file("img_up/azzurra_pagine_slide/220_$foto")) @unlink("img_up/azzurra_pagine_slide/220_$foto");
		if (is_file("img_up/azzurra_pagine_slide/325_$foto")) @unlink("img_up/azzurra_pagine_slide/325_$foto");
		if (is_file("img_up/azzurra_pagine_slide/400_$foto")) @unlink("img_up/azzurra_pagine_slide/400_$foto");
		if (is_file("img_up/azzurra_pagine_slide/710_$foto")) @unlink("img_up/azzurra_pagine_slide/710_$foto");
	}
...
?>
	<script language="javascript">		
		window.location="admin.php?cmd=azzurra_pagine_slide<?php echo $rif;?>";
	</script>
<?php 
} 
...
if($azione=="modifica_testo" && $id_canc!="") {
	...
	$query_up="UPDATE azzurra_pagine_slide SET testo='$testo_foto' WHERE id='$id_canc'";
	$risu_up=$open_connection->connection->query($query_up);
}
if($azione=="cancella_testo" && $id_canc!="") {
	$query_up="UPDATE azzurra_pagine_slide SET testo=NULL WHERE id='$id_canc'";
	$risu_up=$open_connection->connection->query($query_up);
}
...
if($azione=="modifica_testo_eng" && $id_canc!="") {
	...
	$query_up="UPDATE azzurra_pagine_slide SET testo_eng='$testo_foto_eng' WHERE id='$id_canc'";
	$risu_up=$open_connection->connection->query($query_up);
}
if($azione=="cancella_testo_eng" && $id_canc!="") {
	$query_up="UPDATE azzurra_pagine_slide SET testo_eng=NULL WHERE id='$id_canc'";
	$risu_up=$open_connection->connection->query($query_up);
}
?>
<script type="text/javascript">
	...
	function aggiungi_lista(id_check, id_campo){
		...
		if(lista_del!=""){
			...
			document.getElementById('cancella_sel').href='admin.php?cmd=azzurra_pagine_slide<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_del;
		}else{
	...
	function aggiugni_tutti(){
		...
		if(document.getElementById('check_tutti').checked){
			...
			document.getElementById('cancella_sel').style.display="block";
			document.getElementById('cancella_sel').href='admin.php?cmd=azzurra_pagine_slide<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_tutti;
		}else{
			lista_tutti="";
			ind_check = 1;
...
<?php 
$query_tit="SELECT titolo FROM azzurra_pagine WHERE id='$pagina'";
...
<div class="mws-panel grid_8">
	...
	<div style="display:flex; justify-content:space-between;">
		<a href="admin.php?cmd=azzurra_pagine<?php echo $rif;?>" style="color:#7a7a7a">
		...
		<a href="admin.php?cmd=azzurra_pagine_slide_ins<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>">
	...
	<div class="mws-panel-header" style="clear:both">
		<span><i class="icon-table"></i> Elenco Slide</span>
	</div>
			...
			<?php 
				...
				{					  
					...
					{						
						...
						if(file_exists("img_up/azzurra_pagine_slide/s_$foto")) $ante = "img_up/azzurra_pagine_slide/s_$foto";
						elseif(file_exists("img_up/azzurra_pagine_slide/$foto")) $ante = "img_up/azzurra_pagine_slide/$foto";
						else $ante = "https://www.yccs.it/resarea/img_up/azzurra_pagine_slide/$foto";
			?>
				<script type="text/javascript">
				...
				<tr>
					...
					<td align="left" valign="center">
						<?php /*<form name="cambia_testo_<?php echo $id_campo;?>" action="admin.php" method="GET">
							<input type="hidden" name="cmd" value="azzurra_pagine_slide"/>
							...
							<a href="admin.php?cmd=azzurra_pagine_slide&azione=cancella_testo&id_canc=<?php echo $id_campo;?><?php  echo $rif; ?>" style="color:#333333"><i class="fa fa-eraser"></i></a>
						</form>
						<form name="cambia_testo_eng_<?php echo $id_campo;?>" style="padding-top:5px" action="admin.php" method="GET">
							...
							<i class="fa fa-pencil-square-o" style="cursor:pointer;" onclick="document.cambia_testo_<?php echo $id_campo;?>.submit();"></i>
							<a href="admin.php?cmd=azzurra_pagine_slide&azione=cancella_testo_eng&id_canc=<?php echo $id_campo;?><?php  echo $rif; ?>" style="color:#333333"><i class="fa fa-eraser"></i></a>
						</form>*/?>
					</td>
					<td  align="center" valign="center">
						<span class="btn-group">
							<a href="admin.php?cmd=azzurra_pagine_slide&id_canc=<?php  echo $id_campo; ?>&azione=primo<?php  echo $rif; ?>" class="btn btn-small"><i class="icon-arrow-up-2"></i></a>
							<a href="admin.php?cmd=azzurra_pagine_slide&id_canc=<?php  echo $id_campo; ?>&azione=sali<?php  echo $rif; ?>" class="btn btn-small"><i class="icon-arrow-up"></i></a>
							<a href="admin.php?cmd=azzurra_pagine_slide&id_canc=<?php  echo $id_campo; ?>&azione=scendi<?php  echo $rif; ?>" class="btn btn-small"><i class="icon-arrow-down"></i></a>
							<a href="admin.php?cmd=azzurra_pagine_slide&id_canc=<?php  echo $id_campo; ?>&azione=ultimo<?php  echo $rif; ?>" class="btn btn-small"><i class="icon-arrow-down-2"></i></a>
							...
							<!--<a href="admin.php?cmd=slide_mod&id_rec=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-pencil"></i></a>-->
							<a onclick="return confirm('Confermi la cancellazione?')" href="admin.php?cmd=azzurra_pagine_slide&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php  echo $rif; ?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
