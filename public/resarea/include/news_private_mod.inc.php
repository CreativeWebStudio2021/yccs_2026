<?php 
$query_rec = "select * from news_private where id='$id_rec'";
...
if($campocanc!="")
{
	$risu_img = $open_connection->connection->query("select $campocanc from news_private where id='$id_rec'");
	list($cancimg) = $risu_img->fetch();
	
	if(is_file("img_up/news_private/$cancimg")){unlink("img_up/news_private/$cancimg");}
	if(is_file("img_up/news_private/s_$cancimg")){unlink("img_up/news_private/s_$cancimg");}
	if(is_file("files/news_private/$cancimg")){unlink("files/news_private/$cancimg");}
	
	$query_canc_img = "update news_private set $campocanc='' where id='$id_rec'";
	$open_connection->connection->query($query_canc_img);
?>
	<script language="javascript">
		window.location='admin.php?cmd=news_private_mod&id_rec=<?php  echo $id_rec; ?><?php echo $rif;?>';
	</script>	
<?php 
}
...
if($stato=="inviato")
{
	...
	$oggetto_admin->modifica_campi ("news_private" ,$id_rec , $arr_no ,  $arr_thumb, "img_up/news_private");
	
	if ($data_mod!="") $open_connection->connection->query("update news_private set data_news='$data_mod' where id='$id_rec'");
	
	$query_f="SELECT img FROM news_private WHERE id='$id_rec'";
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px"><b>Modifica news private</b></div>
	...
	<div class="mws-panel-body no-padding">
		<form name="gino" class="mws-form" action="admin.php?cmd=news_private_mod&id_rec=<?php  echo $id_rec; ?><?php echo $rif;?>" method="post" enctype="multipart/form-data">
			...
			<div class="mws-form-row">
				...
				<div class="mws-form-item">
					...
					<a href="admin.php?cmd=news_private_mod&id_rec=<?php  echo $id_rec; ?>&campocanc=titolo_eng<?php echo $rif;?>">
			...
			<?php 
			$oggetto_admin->campo_mod("Foto<br /><i>(Dim. 150x75 pixel)</i>" , "img" , "$n_foto"  , "4", 'no', "$cmd", "$id_rec","","","img_up/news_private");
			?>
				<div class="mws-form-row">
				...
				<div class="mws-form-row">
					...
					<div class="mws-form-item">
						...
						<a href="admin.php?cmd=news_private_mod&id_rec=<?php  echo $id_rec; ?>&campocanc=testo_eng<?php echo $rif;?>">
							<i style="font-size:1.5em;" class="fa fa-eraser" aria-hidden="true"></i>
						</a>
					</div>
				</div>
				...
				<div style="margin-left:20px; padding-bottom:10px;">
					<i style="font-size:1.5em;" class="fa fa-eraser" aria-hidden="true"></i> <i>cliccando sulla gomma che compare vicino ad un campo si cancella il contenuto del campo stesso</i></div>
			</div>
