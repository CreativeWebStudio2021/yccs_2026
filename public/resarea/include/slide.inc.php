...
{	
	...
	if ($risu_canc_img) {
		...
		if (is_file("img_up/slide/$img")) @unlink("img_up/slide/$img");
		if (is_file("img_up/slide/s_$img")) @unlink("img_up/slide/s_$img");
		if (is_file("files/slide/$video")) @unlink("files/slide/$video");
	}
...
if ($id_canc) {
	...
	if($azione=="attiva") $open_connection->connection->query("update slide set visibile='1' where id='$id_canc'") ;
	if($azione=="disattiva") $open_connection->connection->query("update slide set visibile='0' where id='$id_canc'") ;
}
		...
		<table class="mws-datatable-fn mws-table">
			<thead>
				<tr>
					...
					<th style="width:200px">Foto/Video</th>
			...
			?>
				...
				<tr>
					...
					<td style="position:relative">
						<?php 
						if(file_exists("img_up/slide/s_$foto")) $ante = "img_up/slide/s_$foto";
						elseif(file_exists("img_up/slide/$foto")) $ante = "img_up/slide/$foto";
						else $ante = "https://www.yccs.it/resarea/img_up/slide/$foto";
						?>
						<img src="<?php echo $ante;?>" style="width:100%"/>
						<?php  if ($video!="" && is_file("files/slide/$video")) { ?><div style="position:absolute;top:40%;left:43%"><i class="icon-play" style="color:#000;font-size:2.0em"></i></div><?php  } ?>
					</td>
					<td><?php  echo $dida; ?></td>
					<!--<td><?php  echo $link; ?></td>-->
					<td align="center" valign="center">
						<?php   
						if ($vis==1) {?> 
							<a href="admin.php?cmd=slide&azione=disattiva&id_canc=<?php echo $id_campo.$rif;?>&pag_att=<?php echo $pag_att;?>" title="Rendi non visibile">
								<i class="fa fa-check-circle fa-2x" aria-hidden="true"></i>
							</a>
						<?php }else{?>
							<a href="admin.php?cmd=slide&azione=attiva&id_canc=<?php echo $id_campo.$rif;?>&pag_att=<?php echo $pag_att;?>" title="Rendi visibile">
