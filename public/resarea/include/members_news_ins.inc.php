...
<script language="javascript">
	function verifica(){
		...
		else if (document.gino.data_news.value=="") alert('Data obbigatoria');
...
if($stato=="inviato"){
	...
	if (isset($_POST['data_news'])) $_POST['data_news'] = $oggetto_admin->date_to_data($_POST['data_news']);
		else $_POST['data_news'] = "";
	
	$oggetto_admin->inserisci_campi ("news" , $arr_no ,  $arr_thumb);
	
	$id_ultimo = $open_connection->connection->lastInsertId();
	
	$query_f="SELECT img FROM news WHERE id='$id_ultimo'";
...
<div class="mws-panel grid_8">
	...
	<div class="mws-panel-body no-padding">
		<form name="gino" class="mws-form" action="admin.php?cmd=members_news_ins<?php echo $rif;?>" method="post" enctype="multipart/form-data">
			<!--<div id="mws-validate-error" class="mws-form-message error" style="display:none;"></div>-->
			<input type="hidden" name="stato" value="inviato">
			<input type="hidden" name="tipo" value="news_young">
<?php 
			...
			echo "<input type='hidden' name='ordine_YA' value='$ord'>";	
?>
				...
				<div class="mws-form-inline">
					<div class="mws-form-row">
						...
						<div class="mws-form-item">
							<input type="text" name="data_news" class="mws-datepicker large"  value="" readonly="readonly" style="width:20%">
						</div>
					</div>
				</div>
				<?php 
