...
if($azione=="cancella" && $id_canc!=""){
	...
	$query_canc_doc = "select file,file_eng from rassegna_doc where id='$id_canc'";
	...
	if ($risu_canc_doc) {
		...
		for ($f=0; $f<$num_canc_doc; $f++) {
			...
			if (is_file("files/rassegna/doc/$doc")) @unlink("files/rassegna/doc/$doc");
			if (is_file("files/rassegna/doc/$doc_eng")) @unlink("files/rassegna/doc/$doc_eng");
		}
	}
				
	$query_canc = "delete from rassegna_doc where id='$id_canc'";
	$risu_canc = $open_connection->connection->query($query_canc);
	
?>
	<script language="javascript">		
		window.location="admin.php?cmd=rassegna_doc<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>";
	</script>
<?php 
} 

if ($id_canc) {
	if($azione=="sali") $oggetto_admin->sali("rassegna_doc", "$id_canc", "id_rassegna" , "$id_rife") ;
	if($azione=="scendi") $oggetto_admin->scendi("rassegna_doc", "$id_canc", "id_rassegna" , "$id_rife") ;
	if($azione=="primo") $oggetto_admin->primo("rassegna_doc", "$id_canc", "id_rassegna" , "$id_rife");
	if($azione=="ultimo") $oggetto_admin->ultimo("rassegna_doc", "$id_canc", "id_rassegna" , "$id_rife");	
	if($azione=="cambia") {
		if(isset($_GET['new_pos'])) $new_pos=$_GET['new_pos']; else $new_pos="";
		if($new_pos!="") $oggetto_admin->cambia("rassegna_doc", "$id_canc", "$new_pos", "id_rassegna" , "$id_rife");			
	}
...
if($azione=="cancella_sel") {
	...
	for($z=0; $z<count($temp)-1; $z++){		
		$query_canc_doc = "select file,file_eng from rassegna_doc where id='".$temp[$z]."'";
		$risu_canc_doc = $open_connection->connection->query($query_canc_doc);
		if ($risu_canc_doc) {
			$num_canc_doc = $risu_canc_doc->rowCount();
			for ($f=0; $f<$num_canc_doc; $f++) {
		...
		$query_canc = "delete from rassegna_doc where id='".$temp[$z]."'";
		$risu_canc = $open_connection->connection->query($query_canc);
	}?>
		<script type="text/javascript">
			window.location='admin.php?cmd=rassegna_doc<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>';
...
$anno_ed = "";
$query_ed = "select anno from rassegna where id='$id_riferimento'";
...
<script type="text/javascript">
	...
	function aggiungi_lista(id_check, id_campo){
		...
		if(lista_del!=""){
			...
			document.getElementById('cancella_sel').href='admin.php?cmd=rassegna_doc<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_del;
		}else{
	...
	function aggiugni_tutti(){
		...
		if(document.getElementById('check_tutti').checked){
			...
			document.getElementById('cancella_sel').style.display="block";
			document.getElementById('cancella_sel').href='admin.php?cmd=rassegna_doc<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>&azione=cancella_sel&lista='+lista_tutti;
		}else{
			lista_tutti="";
			ind_check = 1;
...
<div class="mws-panel grid_8">
	<div style="height:50px;font-size:1.2em;padding-top:10px">Documenti della Conferenza Stampa <b><?php  echo ucfirst($nome_reg); ?></b> </div>
	...
	<div style="display:flex; justify-content:space-between;">
		...
		<a href="admin.php?cmd=rassegna_doc_ins<?php echo $rif;?>&pag_att=<?php echo $pag_att;?>">
	...
	<div class="mws-panel-body no-padding">
		<table class="mws-datatable-fn mws-table">
			...
			<tbody>
			<?php 
				$query_ele = "select * from rassegna_doc where id_rassegna='$id_rife'  order by ordine desc";
				...
				{					  
					...
					$query_ele = "select * from rassegna_doc where id_rassegna='$id_rife' order by ordine desc LIMIT $start,$rec_pag";
					$risu_ele = $open_connection->connection->query($query_ele);
				...
				<tr>
					...
					<td style="width:10%" align="center" valign="center">
						<span class="btn-group">
							<a href="admin.php?cmd=rassegna_doc&id_canc=<?php  echo $id_campo; ?>&azione=primo<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up-2"></i></a>
							<a href="admin.php?cmd=rassegna_doc&id_canc=<?php  echo $id_campo; ?>&azione=sali<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-up"></i></a>
							<a href="admin.php?cmd=rassegna_doc&id_canc=<?php  echo $id_campo; ?>&azione=scendi<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-down"></i></a>
							<a href="admin.php?cmd=rassegna_doc&id_canc=<?php  echo $id_campo; ?>&azione=ultimo<?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-arrow-down-2"></i></a>
							<div class="btn btn-small" style="position:relative; width:15px; height:20px; ">
								...
								<div style="position:absolute; width:20px; height:20px; top:-2px; left:7px; z-index:100">
									<form action="admin.php" method="GET">
										<input type="hidden" name="cmd" value="rassegna_doc"/>
							...
							<a href="admin.php?cmd=rassegna_doc_mod&id_rec=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-pencil"></i></a>
							<a onclick="return confirm('Cancellare l\'elemento?')" href="admin.php?cmd=rassegna_doc&azione=cancella&id_canc=<?php  echo $id_campo; ?><?php  echo $rif; ?>&pag_att=<?php echo $pag_att;?>" class="btn btn-small"><i class="icon-trash"></i></a>
						</span>
					</td>
				</tr>
			<?php 
