<?php
$_GET['id'] ? $id_contatto = $_GET['id'] : '';
$_GET['lingua'] ? $lingua = $_GET['lingua'] : '';
$_GET['pos'] ? $pos = $_GET['pos'] : '';
?>
<div style="position:absolute; width:80%;  left:10%; top:50px; background:#fff; box-shadow: 4px 4px 25px 0px rgba(0, 0, 0, 0.1);">
		...
		<form name="sendRequest" action="{{ url()->full() }}" method="post" autocomplete="off">
			...
			<div style="display:flex; flex-direction:column; gap:10px">
				...
				<div style="display:flex; gap:10px; margin-top:20px; margin-bottom:10px;justify-content:center;">
					...
					<button class="btnYccsAnnulla" style="width:100px;" type="button" onclick="vediBoardRequest('<?=$pos;?>');"><?php if($lingua=="ita"){?>ANNULLA<?php }else{?>BACK<?php }?></button>
				</div>
			</div>
		</form>
	</div>
