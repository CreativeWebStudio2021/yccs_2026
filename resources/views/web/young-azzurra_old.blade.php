...
@if ($num_s>0) 
	...
	<!-- Inspiro Slider -->
	<div id="slider" class="inspiro-slider slider-fullscreen dots-creative solo_desk" data-fade="true" style="margin-top:-30px;">
		@foreach ($query_s as  $key=>$campo)
			<!-- Slide 1 -->
			<div class="slide kenburns" data-bg-image="resarea/img_up/ya_slide/{{$campo->img}}">
					...
					<div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
						<div class="col-lg-12 col-md-12 col-xs-12" style="padding:0;">
							@php
								...
								$query_news = $query_news->orderBy('ordine', 'DESC');
							...
							@foreach($query_news AS $key_news=>$value_news)
								...
								<div class="portfolio-item img-zoom">
									<div class="portfolio-item-wrap">
										@if($img!="")
											<div class="portfolio-image" style="margin-bottom:10px;">
												<a href="{{ $link_news }}" title="{{ $titolo }} - NEWS - {{ config('app.name') }}">
													<picture>
													  <source srcset="https://www.yccs.it/<?php if(str_replace("http://","",$img_m)==$img_m){?>{{ config('app.url') }}/<?php }?><?php echo $img_m;?>" media="(max-width: 350px)" />
													  ...
													  <img src="https://www.yccs.it/<?php if(str_replace("http://","",$img)==$img){?>{{ config('app.url') }}/<?php }?><?php echo $img;?>"  alt="{{ $titolo }} - NEWS - {{ config('app.name') }}"/>
													</picture>	
												</a>
											</div>
										@endif								
									...
									<div class="portfolio-item-wrap" style="">
										<div class="row">
											<div class="col-lg-4 col-md-4 col-sm-4bis col-xs-4">
												@if($img!="")
													<div class="portfolio-image" style="margin-bottom:5px;">
														<a href="{{ $link_news }}" title="{{ $titolo }} - NEWS - {{ config('app.name') }}">
															<picture>
															  <source srcset="https://www.yccs.it/<?php if(str_replace("http://","",$img_m)==$img_m){?>{{ config('app.url') }}/<?php }?><?php echo $img_m;?>" media="(max-width: 350px)" />
															  <source srcset="https://www.yccs.it/<?php if(str_replace("http://","",$img_l)==$img_l){?>{{ config('app.url') }}/<?php }?><?php echo $img_l;?>" media="(max-width: 450px)" />
	...
	<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
		<div style=" padding:20px 40px; margin:0; border:0;padding-bottom:20px; background:#F7F7F7">
			...
			<div class="row">
					...
					<div class="row team-members">
						@foreach($query_gal AS $key_gal=>$value_gal)
							@php
								...
								$link_gal="young-azzurra/team/".creaSlug($value_gal->nome,"")."_".creaSlug($value_gal->cognome,"")."-".$value_gal->id.".html";							
							@endphp
							<div class="col-lg-{{ $col3 }}">
								<div class="team-member">
									<div class="team-image">
										<a href="<?php echo $link_gal;?>" title="<?php echo $nome;?> <?php echo $cognome;?>, <?php echo $titolo;?> - Team - Young Azzurra">
											<img src="https://www.yccs.it/<?php  echo $dir_up; ?>/ya_team/<?php if(file_exists(public_path()."/resarea/img_up/ya_team/s_".$foto)) echo "s_";?><?php echo $foto;?>"  alt="<?php echo $nome;?> <?php echo $cognome;?>, <?php echo $titolo;?> - Team - Young Azzurra" style="width:100%">
										</a>
									</div>
									<div class="team-desc">
										...
										<a class="read-more" style="font-size:14px; color:#111; font-family:'Open Sans'; font-weight:700" title="<?php echo $nome;?> <?php echo $cognome;?>, <?php echo $titolo;?> - Team - Young Azzurra" href="<?php echo $link_gal;?>"><?php if($lingua=="ita"){?>VEDI PROFILO<?php }else{?>SEE PROFILE<?php }?></a>
									</div>
								</div>
							</div>
						@endforeach
				...
				@endif
				<div style="width:100%">
					<a href="<?php if($lingua=="eng"){?>en/<?php }?>young-azzurra/team.html">
						<div style="clear:both; width:120px; margin:0 auto; background:#8c8c8c; color:#fff; border-radius:4px; text-align:center; cursor:pointer; margin-bottom:15px;">
...
@php
	...
	if($ris==1) $foto_riga=2; else $foto_riga=$num_gal;
	if($ris==1) {$col_foto = "6";$col_foto_span = "";}
	else{
		if($num_gal==4){$col_foto = "12";$col_foto_span = "";}
		elseif($num_gal==3){$col_foto = "8";$col_foto_span = "2";}
		elseif($num_gal==2){$col_foto = "6";$col_foto_span = "3";}
		elseif($num_gal==1){$col_foto = "4";$col_foto_span = "4";}
	}
@endphp
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
				<div style=" padding:20px 40px; margin:0; border:0;padding-bottom:20px; background:#F7F7F7">
					...
					@endif
							...
							<div class="grid-layout post-{{ $num_v }}-columns m-b-10" data-item="post-item">
								@foreach($query_gal AS $key_gal=>$value_gal)
									@php
										$titolo = $value_gal->titolo;
										if($lingua=="eng" && $value_gal->titolo_eng && trim($value_gal->titolo_eng)!="") $titolo = $value_gal->titolo_eng;											
									...
									<div class="post-item border">
										<div class="post-item-wrap">
											<a href="<?php if($lingua=="eng") echo "en/";?><?php  echo $link_gal; ?>" title="<?php  echo $titolo_gal; ?> - Video Gallery - Young Azzurra"> 
												<div class="post-image" style="position:relative; text-align:center; background:#F7F7F7;"> 
													@if($value_gal->video_fb)
														<div style="position:relative;  width:265px; margin:0 auto;">
															<div style="position:absolute; width:100%; height:100%; z-index:100"></div>
															<div class="fb-video" data-href="https://www.facebook.com/facebook/videos/<?php echo $value_gal->video_fb;?>/"  data-show-text="false" style="width:100%;">
													...
													@else
														...
														@endphp
														<img src="https://www.yccs.it/<?php echo $foto_gal;?>" style="width:265px; margin:0 auto;" alt="<?php echo $titolo;?> - Video Gallery - Young Azzurra"/>
													@endif														
												</div>														
												<div class="post-content-details" style=" background:#F7F7F7; text-align:Center; padding-top:10px">
													<div class="post-title" style="white-space:wrap" style="text-align:center !important;">
														<a href="<?php echo $link_gal;?>" title="<?php echo $titolo;?> - Video Gallery - Young Azzurra">
															<h3 style="font-size:20px"><?php echo $titolo;?></h3>
														</a>
														<a class="read-more" style="font-size:14px; color:#111; font-family:'Open Sans'; font-weight:700" title="<?php echo $titolo;?> - Video Gallery - Young Azzurra" href="<?php echo $link_gal;?>"><?php if($lingua=="ita"){?>Vedi Video<?php }else{?>See Video<?php }?></a>
