					...
					<div style="width:185px; margin-top:20px; justify-content:flex-end; display:flex; gap:2px; flex-direction:column">
							...
							@elseif($query_prog[0]->tipo_link=="allegato" && $pdf!="")
								@php
									...
									if($query_prog[0]->link_fisso=="1"){
										...
										$link_p .= $anno_edh."/".creaSlug($nome_regata_edh,"")."-".$query_prog[0]->id."/info-".$query_prog[0]->id."/".creaSlug($testo_link,"");
									}
								@endphp
							...
							@if ($query_iscritti[0]->tipo_link=="link" && $link!="")
								@php
									if($query_iscritti[0]->link_fisso=="1"){
										...
										$link .= $anno_edh."/".creaSlug($nome_regata_edh,"")."-".$query_iscritti[0]->id."/iscritti-".$query_iscritti[0]->id."/".creaSlug($testo_link,"");
									}
								@endphp	
							...
							@elseif($query_iscritti[0]->tipo_link=="allegato" && $pdf!="")
								@php
									...
									if($query_iscritti[0]->link_fisso=="1"){
										...
										$link_i .= $anno_edh."/".creaSlug($nome_regata_edh,"")."-".$id_edh."/iscritti-".$query_iscritti[0]->id."/".creaSlug($testo_link,"");
									}
								@endphp
						...
						@php
							$query_documenti = DB::table('edizioni_doc')
								...
								->whereIn('testo_link', ['Bando di Regata', 'Pre_Bando di Regata'])
								->orderByRaw("CASE WHEN testo_link = 'Bando di Regata' THEN 0 ELSE 1 END")
								->first();
						@endphp
						...
						@if($query_documenti)
							@php
								...
								// Normalizza etichetta: se arriva "Pre_Bando di Regata" mostra sempre "Bando di Regata"
								$testo_link = str_replace('Pre_', '', (string)$testo_link);
								...
								if ($query_documenti->tipo_link == "link" && $link) {
									...
									if($query_documenti->link_fisso==1){
										$link_bando = "regate-";
										if($lingua=="eng") $link_bando = "en/regattas-";
										$link_bando .= $anno_edh."/".creaSlug($nome_regata_edh,"")."-".$id_edh."/doc-".$query_documenti->id."/".creaSlug($testo_link,"");
									}
								}
							@endphp
						...
						@else
							@php
								$link_modulo_attivo=0;
								$query_mod = DB::table('edizioni_doc');
								$query_mod = $query_mod->select('*');
								$query_mod = $query_mod->where('id_edizione','=',$id_edh);
								$query_mod = $query_mod->where('modulo','=','1');
								$query_mod = $query_mod->get();
								if($query_mod->count()>0){
									$link_modulo_attivo=1;		
									$testo_link = $query_mod[0]->testo_link_eng;
									if($lingua=="ita" && isset($query_mod[0]->testo_link) && $query_mod[0]->testo_link!="") $testo_link = $query_mod[0]->testo_link;
								}
							@endphp
							@if($link_modulo_attivo==1)
								@php
									if($query_mod[0]->link_fisso==1){
										$link_modulo = "regate-";
										if($lingua=="eng") $link_modulo = "en/regattas-";
										$link_modulo .= $anno_edh."/".creaSlug($nome_regata_edh,"")."-".$id_edh."/doc-".$query_mod[0]->id."/".creaSlug($testo_link,"");
									}
								@endphp
								<a href="{{ $link_modulo }}" target="_blank">	
									<div class="list-link">
										<img src="{{ asset('web/images/star_grey.png') }}" alt="" class="list-icon"/>
										<span style="font-size:12px;">
											<?php if($lingua=="ita"){?>
